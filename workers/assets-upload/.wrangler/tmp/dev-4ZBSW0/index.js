...
// .wrangler/tmp/bundle-gc4q5a/checked-fetch.js
...
// ../../../../../.nvm/versions/node/v20.11.1/lib/node_modules/wrangler/templates/modules-watch-stub.js
...
// .wrangler/tmp/bundle-gc4q5a/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-gc4q5a/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
__name(getJPEGDimensions, "getJPEGDimensions");
...
__name(getPNGDimensions, "getPNGDimensions");
function getWebPDimensions(data) {
  if (data.length >= 30 && data[0] === 82 && data[1] === 73 && data[2] === 70 && data[3] === 70 && data[8] === 87 && data[9] === 69 && data[10] === 66 && data[11] === 80) {
    if (data[12] === 86 && data[13] === 80 && data[14] === 56) {
      const width = ((data[26] | data[27] << 8 | data[28] << 16) & 16383) + 1;
      const height = ((data[28] >> 6 | data[29] << 2 | (data[30] & 63) << 10) & 16383) + 1;
      return { width, height };
    }
  }
  return {};
}
__name(getWebPDimensions, "getWebPDimensions");
function generateSafeFilename(originalName) {
  const extension = originalName.match(/\.[^.]+$/)?.[0] || "";
  const nameWithoutExt = originalName.replace(/\.[^.]+$/, "");
  const safeName = nameWithoutExt.toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "") || "asset";
...
__name(generateSafeFilename, "generateSafeFilename");
async function handleUpload(request, env) {
  try {
    ...
    const assetData = {
      ...
      height: height || null,
      duration: null,
      // TODO: Add duration detection for audio/video
      is_public: isPublic,
      user_id: userId || null
    };
...
async function handleGetAsset(request, env, assetId) {
  try {
...
__name(handleGetAsset, "handleGetAsset");
...
__name(handleListAssets, "handleListAssets");
async function handleUpdateAsset(request, env, assetId) {
  try {
    ...
    if (updates.categories !== void 0) updateData.categories = updates.categories;
    if (updates.tags !== void 0) updateData.tags = updates.tags;
    if (updates.isPublic !== void 0) updateData.is_public = updates.isPublic;
    const { data, error } = await supabase.from("assets").update(updateData).eq("id", assetId).select().single();
    if (error) {
      return Response.json({
        success: false,
...
async function handleDeleteAsset(request, env, assetId) {
  try {
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
    const { data: asset, error: getError } = await supabase.from("assets").select("file_path").eq("id", assetId).single();
    if (getError || !asset) {
      return Response.json({
        success: false,
        error: "Asset not found"
...
__name(handleDeleteAsset, "handleDeleteAsset");
