...
/**
 * Get available languages (stub implementation)
 */
function getAvailableLanguages(): string[] {
  // This would normally come from the lazy-loader, but we'll provide a basic implementation
  return ['en', 'nl', 'de', 'fr', 'es', 'it', 'pt', 'sv', 'da', 'no', 'fi']
}

/**
 * Load language with base merging (stub implementation)
 */
async function loadLanguageWithBase(locale: string): Promise<any> {
  // This would normally load from lazy-loader, but we'll provide a basic implementation
  console.log(`[i18n] Loading language: ${locale}`)
  
  // Return empty object for now - this should be implemented based on your i18n system
  return {}
}
