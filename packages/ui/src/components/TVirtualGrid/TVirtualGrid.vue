...
const visibleRange = computed(() => {
  ...
  const viewportHeight = scrollableParent === window
    ? windowHeight.value
  ...
  const visibleEndY = Math.min(totalHeight.value, viewportBottom - containerOffsetTop)

  const startRow = Math.max(0, Math.floor(visibleStartY / rowHeight) - props.bufferRows)
  ...
  rafId = requestAnimationFrame(() => {
    ...
    if (containerRef.value) {
      ...
      if (scrollableParent === window) {
        ...
        containerTop.value = rect.top - parentRect.top + (scrollableParent as HTMLElement).scrollTop
      }
    }
...
const updateDimensions = () => {
  if (containerRef.value) {
    ...
    if (scrollableParent === window) {
      containerTop.value = rect.top + (window.scrollY || window.pageYOffset || document.documentElement.scrollTop)
      scrollTop.value = window.scrollY || window.pageYOffset || document.documentElement.scrollTop
    } else {
  ...
  windowHeight.value = scrollableParent === window
    ? window.innerHeight
    : (scrollableParent as HTMLElement).clientHeight
}
...
onMounted(async () => {
  ...
  if (containerRef.value) {
    ...
    console.log('Found scrollable parent:', scrollableParent === window ? 'window' : 'element')
  }
  ...
  if (scrollableParent === window) {
    ...
    ;(scrollableParent as HTMLElement).addEventListener('scroll', handleParentScroll, { passive: true })
  }
...
onUnmounted(() => {
  ...
  if (scrollableParent === window) {
    ...
    (scrollableParent as HTMLElement).removeEventListener('scroll', handleParentScroll)
  }
