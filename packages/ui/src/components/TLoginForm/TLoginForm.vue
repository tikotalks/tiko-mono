...
const isVerificationValid = computed(() => {
  return verificationCode.value.length === codeLength && /^\d+$/.test(verificationCode.value)
})
...
const goBackToEmail = () => {
  currentStep.value = currentStep.value === 'verification' && fullName.value ? 'register' : 'email'
  verificationCode.value = ''
  verificationError.value = ''
  clearCooldown()
}
