<template>
	<div
		:class="bemm('', ['', tikoConfig.isApp ? 'is-app' : 'is-website'])"
		:style="deviceTiltStyles"
	>
		<TAuthWrapper
		...
		>
			...
			>
				...
				<div
					v-if="tikoConfig.auth.showLoginButton && !isAuthenticated"
					:class="bemm('skip-auth-buttons')"
				>
					<!-- Language switcher button -->
					<TButton
						:class="bemm('language-button')"
						type="ghost"
...
<script setup lang="ts">
	...
	if (!pinia) {
		console.error(
			'[TFramework] Pinia is not initialized. Please ensure app.use(pinia) is called before mounting the app.'
		)
	}
	...
	const currentLanguageName = computed(() => {
		...
		if (isSkipAuth) {
			...
			if (i18nInstance.value) {
				const currentLoc =
					i18nInstance.value.currentLocale?.value ||
					...
					const currentModelValue = computed(() => {
						...
						if (isSkipAuth) {
							try {
								...
								const locale =
									i18nInstance.value?.currentLocale?.value || i18nInstance.value?.locale || 'en'
	...
	watch(currentLanguage, (newLanguage, oldLanguage) => {
		...
		if (newLanguage && newLanguage !== locale.value && newLanguage !== oldLanguage) {
			console.log(
				'[TFramework] Language changed in settings:',
				newLanguage,
				'current locale:',
				locale.value
			)
	...
	const applyTheme = (theme: string) => {
		...
		console.log(
			'[TFramework] Theme applied. data-theme attribute:',
