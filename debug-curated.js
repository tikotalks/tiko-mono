...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhxamlldXZsaGZpc3ZndndnZmJuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MDA2OTc3NjEsImV4cCI6MjAxNjI3Mzc2MX0.KK7-mUZH8QB7Ub2EyZ63T1rbyMD5Mxt8MfwQnFkYVYQ';
  ...
  try {
    ...
    for (const item of othersCurated) {
      if (item.type === 'sequence' && !item.parent_id) {
        ...
        const childResponse = await fetch(`${SUPABASE_URL}/rest/v1/items?${childParams.toString()}`, {
          headers: {
            'Content-Type': 'application/json',
            'apikey': SUPABASE_KEY,
            'Authorization': `Bearer ${session.access_token}`
        ...
        console.log(`   - "${item.name}" has ${children.length} children:`, children.map(c => ({ id: c.id, name: c.name, type: c.type })));
      }
    }
