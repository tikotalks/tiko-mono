...
# Only build if changes are in ui-docs tool, packages/ui, or shared dependencies
if git diff --quiet HEAD^ HEAD -- tools/ui-docs/ packages/ui/ package.json pnpm-lock.yaml; then
  echo "No changes affecting ui-docs tool, skipping build."
  exit 0
else
  echo "Changes detected affecting ui-docs tool, proceeding with build."
  exit 1
fi