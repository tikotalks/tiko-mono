-- ============================================
-- COMPLETE TRANSLATION SYSTEM WITH i18n_ PREFIX (SIMPLIFIED)
-- ============================================
-- This script:
...
-- View for pending translations (for review interface) - SIMPLIFIED WITHOUT EMAIL
CREATE OR REPLACE VIEW i18n_pending_translations AS
SELECT 
    tv.*,
    tv.created_by as created_by_id,
    (
        SELECT value 
...
-- View for translation history - SIMPLIFIED WITHOUT EMAIL
CREATE OR REPLACE VIEW i18n_translation_history AS
SELECT 
    tv.*,
    tv.created_by as created_by_id,
    tv.reviewed_by as reviewed_by_id
FROM i18n_translation_versions tv
ORDER BY tv.key, tv.locale, tv.version_number DESC;

-- View for contributor statistics - SIMPLIFIED WITHOUT EMAIL
...
BEGIN
    ...
    RAISE NOTICE 'NO EMAIL DEPENDENCIES - Uses user IDs only';
END $$;