...
describe('Final Authentication Flow Tests', () => {
  beforeEach(() => {
  ...
  describe('OTP Authentication Flow', () => {
    ...
    it('should handle resend OTP functionality', () => {
      ...
      cy.contains('button', /Resend in \d+s/).should('be.visible').should('be.disabled')
  ...
  describe('Magic Link Authentication', () => {
    it('should authenticate user via magic link in URL hash', () => {
      ...
      // Store session directly to simulate successful auth
      cy.window().then((win) => {
        const session = {
      ...
      // Visit with magic link parameters - this will redirect to /auth/callback
      cy.visit(`/#access_token=${mockToken}&refresh_token=refresh_abc123&expires_in=3600&token_type=bearer`)
      
      // Since we're redirected to /auth/callback, wait for that
      cy.url().should('include', '/auth/callback')
      cy.contains('Processing authentication').should('be.visible')
    ...
    it('should handle malformed magic link tokens', () => {
      ...
      // Will be redirected to auth callback
      cy.url().should('include', '/auth/callback')
      
      // Wait for redirect back with error
      cy.url({ timeout: 10000 }).should('eq', 'http://localhost:3001/')
