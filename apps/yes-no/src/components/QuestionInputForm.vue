<template>
  <div :class="bemm()">
              ...
              <TButton
                ...
                @click.stop="
                  () => {
                    ...
                    yesNoStore.toggleFavorite(item.id)
                  }
                "
                :aria-label="
                  item.is_favorite ? t('yesno.removeFromFavorites') : t('yesno.addToFavorites')
                "
              />
              <TButton
                icon="trash"
                ...
                @click.stop="
                  () => {
                    ...
                    yesNoStore.deleteQuestion(item.id)
                  }
                "
    ...
    <div :class="bemm('actions')">
      <TButton type="outline" color="secondary" @click="emit('close')" size="medium">
        {{ t('common.cancel') }}
      </TButton>
      <TButton
        type="default"
...
<script setup lang="ts">
  ...
  const recentQuestionItems = computed(() =>
    yesNoStore.recentQuestionItems.slice(0, maxShowRecentItems.value)
  )
