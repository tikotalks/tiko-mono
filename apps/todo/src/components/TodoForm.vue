  ...
  <div :class="bemm()">
    <TForm @submit.prevent="handleSubmit" :class="bemm('form')">
      <div :class="bemm('section', ['main'])">
        ...
        <TFormGroup>
          ...
          <div :class="bemm('image-field')">
            <TImageInput v-model="formData.image" :label="t('common.image')"
      ...
      <div :class="bemm('section', ['steps'])">
        <h3 :class="bemm('section-title')">{{ t('todo.step', { count: formData.steps.length }) }}</h3>

        <div :class="bemm('steps')">
          <div v-for="(step, index) in formData.steps" :key="index" :class="bemm('step')">
            <TFormGroup>
              <TFormField :label="`${t('todo.step', { count: 1 })} ${index + 1}`" :name="`step-${index}`" required>
                <TInputText v-model="step.name" :placeholder="t('todo.enterStepName')" />
              </TFormField>

              <div :class="bemm('step-image')">
                <TImageInput v-model="step.image" :label="t('todo.stepImage')"
                  :placeholder="t('common.actions.selectImage')" :show-label="false" />
              </div>
            </TFormGroup>

            <TButton v-if="formData.steps.length > 1" :icon="Icons.TRASH" :color="Colors.ERROR" size="small"
              @click="removeStep(index)" />
          </div>

          <TButton :icon="Icons.ADD_M" :text="t('todo.addStep')" :type="ButtonType.DEFAULT" @click="addStep" />
        </div>
      </div>

      <TFormActions>
        <TButton :type="ButtonType.DEFAULT" @click="handleCancel">{{ t('common.cancel') }}</TButton>
        <TButton html-button-type="submit" :type="ButtonType.DEFAULT" :loading="loading" :disabled="!isValid">{{
