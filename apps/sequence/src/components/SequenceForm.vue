    ...
    <div :class="bemm('section')">
      <TFormGroup>
        ...
        <TImageInput
          ...
          v-model="form.image"
          :color="form.color"
          :placeholder="t('sequence.uploadSequenceImage')"
        ...
        <template v-if="props.showVisibilityToggle">
          ...
          <p v-if="form.isPublic" :class="bemm('visibility-note')">{{ t('common.publicNote') }}</p>
...
<script setup lang="ts">
  ...
  const handleDrop = (event: DragEvent) => {
    ...
    if (draggedIndex.value !== null && dragOverIndex.value !== null) {
      ...
      const insertIndex =
        dragOverIndex.value > draggedIndex.value ? dragOverIndex.value - 1 : dragOverIndex.value
