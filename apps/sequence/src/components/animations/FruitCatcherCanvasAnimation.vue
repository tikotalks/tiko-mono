    ...
    <div :class="bemm('timer')">
      <svg viewBox="0 0 100 100" :class="bemm('timer-svg')">
        ...
        <circle
          ...
          :class="[
            bemm('timer-progress'),
            { [bemm('timer-progress--warning')]: timeRemaining <= 5 },
          ]"
        />
      </svg>
...
<script setup lang="ts">
  ...
  const fruitConfigs = [
    ...
    {
      id: 'd7140ae2-354b-43d4-b008-a6c7ce83bccd',
      name: 'Strawberry',
      sizeMultiplier: 1.2,
      timeValue: 3.5,
    },
    {
      id: '0b99e82c-1002-457b-829d-bbeef97edeeb',
      name: 'Banana',
      sizeMultiplier: 1.5,
      timeValue: 3,
    },
    {
      id: 'eed4ad37-3c27-44a0-9695-d38a4e2f588c',
      name: 'Orange',
      sizeMultiplier: 1.8,
      timeValue: 2.5,
    },
    ...
    {
      id: 'e8ae9a60-3345-48c7-aafb-35a5f2f63e04',
      name: 'Pineapple',
      sizeMultiplier: 3,
      timeValue: 1.5,
    },
  ...
  const handlePointerDown = (event: MouseEvent | TouchEvent) => {
    ...
    for (let i = fallingFruits.length - 1; i >= 0; i--) {
      ...
      const distance = Math.sqrt(
        Math.pow(x - (fruit.x + fruit.size / 2), 2) + Math.pow(y - (fruit.y + fruit.size / 2), 2)
      )
  ...
  const createClickAnimation = (x: number, y: number) => {
    ...
    const clickObj = canvasAnimation.createObject(
      clickId,
      clickId,
      x - size / 2,
      y - size / 2,
  ...
  onMounted(async () => {
    ...
    try {
      ...
      const backgroundVideoUrl = await resolveAssetUrl('b56c93bb-19e2-4a7a-94d7-00039d2bb043', {
        media: 'assets',
      })
