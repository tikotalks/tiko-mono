...
<script setup lang="ts">
  ...
  // Animation state
  let currentPhase = ref<'entering' | 'idle' | 'liftoff' | 'flying' | 'fadeout' | 'complete'>(
    'entering'
  )
  let rocketState = ref({
    x: 0,
  ...
  const startAnimation = async () => {
    ...
    const canvas = canvasAnimation?.getCanvas()
    const logicalWidth = canvas
      ? parseInt(canvas.style.width) || window.innerWidth
      : window.innerWidth
    const logicalHeight = canvas
    ...
    canvasAnimation?.createObject('rocket', 'rocket', rocketX, rocketY, rocketSize, rocketSize)
    ...
    console.log(
      '[RocketCanvas] All objects:',
      canvasAnimation ? Array.from(canvasAnimation.getAllObjects().keys()) : []
    )
  ...
  const loadVideo = (src: string): Promise<HTMLVideoElement> => {
    return new Promise((resolve, reject) => {
      ...
      video.onerror = error => {
        console.error(
          '[RocketCanvas] ❌ First attempt failed with CORS, trying without:',
          src,
          error
        )
  ...
  const startRocketSequence = (
    ctx: CanvasRenderingContext2D,
    width: number,
    height: number,
    bgAsset: HTMLVideoElement | HTMLImageElement,
    isVideo: boolean,
    rocketImg: HTMLImageElement,
    fireImgs: HTMLImageElement[]
  ) => {
    ...
    switch (currentPhase.value) {
      ...
      case 'fadeout':
        ...
        if (timeSinceFadeStart < fadeDuration) {
          ...
          console.log(
            '[RocketCanvas] Fading out... Progress:',
            fadeProgress,
            'Opacity:',
            canvasOpacity.value
          )
  ...
  const drawAnimatedScene = (
    ...
    const assetWidth =
      isVideo && bgAsset instanceof HTMLVideoElement
        ? bgAsset.videoWidth
        : (bgAsset as HTMLImageElement).width
    const assetHeight =
      isVideo && bgAsset instanceof HTMLVideoElement
        ? bgAsset.videoHeight
        : (bgAsset as HTMLImageElement).height
    ...
    console.log(
      '[RocketCanvas] Background flying! Y:',
      bgY,
      'Movement:',
      backgroundState.value.y,
      'Opacity:',
      canvasOpacity.value
    )
  ...
  const drawAnimatedFire = (
    ctx: CanvasRenderingContext2D,
    width: number,
    height: number,
    fireImg: HTMLImageElement
  ) => {
  ...
  const drawWithImages = (
    ...
    console.log(
      '[RocketCanvas] Background aspect ratio:',
      imageAspectRatio,
      '(',
      bgImg.width,
      'x',
  ...
  const drawManualElements = (
    ctx: CanvasRenderingContext2D,
    width: number,
    height: number,
    bgAspectRatio: number = 16 / 9
  ) => {
    ...
    console.log(
      '[RocketCanvas] To animate: background should move from y=' +
        bgY +
        ' to y=' +
        (height - bgHeight)
    )
