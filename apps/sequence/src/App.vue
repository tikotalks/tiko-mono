...
<script setup lang="ts">
  ...
  onMounted(async () => {
    try {
      ...
      // Pre-load all sequence if enabled in settings
      const { useSequenceStore } = await import('./stores/sequence')
      const cardStore = useSequenceStore()
      ...
      if (preloadSequence || isSkipAuth) {
        console.log(
          '[App] Pre-loading all sequence... (skip auth:',
          isSkipAuth,
          'preload setting:',
          preloadSequence,
          ')'
  ...
  const frameworkConfig = computed<FrameworkConfig>(() => {
    // Safely access the sequence settings key
    const sequenceSettingsKey =
      keys.value?.sequence?.sequenceSettings || 'sequence.sequenceSettings'
