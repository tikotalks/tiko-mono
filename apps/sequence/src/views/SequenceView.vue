...
<script setup lang="ts">
  ...
  const isAdmin = computed(() => {
    ...
    return user.email?.endsWith('@admin.tiko.app') || user.user_metadata?.role === 'admin' || false
  })
  ...
  const loadSequence = async () => {
    ...
    if (currentGroupId.value && savedSequence.length === 0) {
      ...
      savedSequence = await sequenceStore.loadSequence(
      ...
      if (savedSequence.length === 0) {
        ...
        if (savedSequence.length === 0 && parentCard) {
          console.warn(
            '[SequenceView] No items found after multiple attempts. Parent card details:',
            {
              id: parentCard.id,
    ...
    if (savedSequence.length > 0 || sequence.value.some(c => c.id.startsWith('ghost-'))) {
      ...
      console.warn(
        '[SequenceView] Received empty array but have existing data, keeping current sequence'
      )
    ...
    if (savedSequence.length > 0) {
      ...
      const sequencesWithSpeech = savedSequence.filter(
        card => !card.id.startsWith('empty-') && card.speech
      )

      if (sequencesWithSpeech.length > 0) {
        console.log(
          `[SequenceView] Pre-generating speech for ${sequencesWithSpeech.length} sequences`
        )
        ...
        await Promise.all(
          sequencesWithSpeech.map(card =>
            preloadAudio(card.speech || '', language).catch(err =>
              console.warn(
                `[SequenceView] Failed to pre-generate speech for sequence ${card.id}:`,
                err
              )
            )
          )
      ...
      const sequenceTiles = savedSequence.filter(
        card => card.type === 'sequence' && !card.id.startsWith('empty-')
      )

      if (sequenceTiles.length > 0) {
        console.log(
          '[SequenceView] Loading children for',
          sequenceTiles.length,
          'sequence tiles in background'
        )
        ...
        Promise.all(
          sequenceTiles.map(async card => {
            try {
              ...
              const childrenWithSpeech = children.filter(
                child => child.speech && child.type !== 'sequence'
              )
              if (childrenWithSpeech.length > 0) {
                console.log(
                  `[SequenceView] Pre-generating speech for ${childrenWithSpeech.length} items in sequence ${card.id}`
                )
                const language = currentLocale.value.split('-')[0]
                ...
                await Promise.all(
                  childrenWithSpeech.map(child =>
                    preloadAudio(child.speech, language).catch(err =>
                      console.warn(
                        `[SequenceView] Failed to pre-generate speech for child ${child.id}:`,
                        err
                      )
                    )
                  )
  ...
  const handleSaveSequence = async (
    ...
    console.log(
      '[SequenceView] Saving sequence:',
      formData.title,
      'with reward animation:',
      formData.rewardAnimation
    )
    try {
      if (isNewCard) {
        ...
        if (sequenceId) {
          ...
          const emptyCardIndex = updatedSequence.findIndex(
            c => c.id.startsWith('empty-') && c.index === index
          )
          if (emptyCardIndex >= 0) {
          ...
          if (formData.items && formData.items.length > 0) {
            ...
            if (itemsWithSpeech.length > 0) {
              console.log(
                `[SequenceView] Pre-generating speech for ${itemsWithSpeech.length} new items`
              )
              const language = currentLocale.value.split('-')[0]
              ...
              await Promise.all(
                itemsWithSpeech.map(item =>
                  preloadAudio(item.speech, language).catch(err =>
                    console.warn(
                      `[SequenceView] Failed to pre-generate speech for item ${item.id}:`,
                      err
                    )
                  )
                )
        ...
        if (freshCard) {
          await sequenceStore.updateCardInCache(
            freshCard,
            currentGroupId.value,
            currentLocale.value
          )
  ...
  const duplicateItem = async (originalCard: SequenceTile) => {
    try {
      ...
      const items =
        originalCard.type === 'sequence' ? await sequenceService.loadSequence(originalCard.id) : []

      console.log('[SequenceView] Loaded', items.length, 'items for duplication')
      if (items.length === 0) {
        console.warn(
          '[SequenceView] WARNING: No items found for curated sequence. This means children are not loading!'
        )
      } else {
        console.log(
          '[SequenceView] Items loaded:',
          items.map(item => ({ id: item.id, title: item.title, type: item.type }))
        )
      }
      ...
      if (sequenceId) {
        ...
        const updatedSequence = [...sequence.value]
        const emptyCardIndex = updatedSequence.findIndex(
          c => c.id.startsWith('empty-') && c.index === targetIndex
        )
        if (emptyCardIndex >= 0) {
          updatedSequence[emptyCardIndex] = newSequence
        } else {
        ...
        if (duplicateData.items && duplicateData.items.length > 0) {
          ...
          const childrenTiles: SequenceTile[] = duplicateData.items.map(
            (item: any, idx: number) => ({
  ...
  const openCardEditForm = async (card: SequenceTile, index: number) => {
    ...
    // Calculate ownership
    const isOwner = isNewCard
      ? true
      : card.ownerId === authStore.user?.id || card.user_id === authStore.user?.id
    const isCurated = card.isCurated || false
  ...
  const handleTileAction = async (tile: SequenceTile) => {
    ...
    if (hasChildren) {
      ...
      const speakLocale =
        (tile as any).effective_locale || (tile as any).base_locale || currentLocale.value
  ...
  const tilesWithChildren = computed(() => {
    ...
    console.log(
      '[SequenceView] tilesWithChildren computed:',
      hasChildren.size,
      'tiles have children'
    )
    return hasChildren
  ...
  const handleCardDrop = async (droppedCard: SequenceTile, targetCard: SequenceTile) => {
    ...
    try {
      ...
      console.log(
        `Moved "${droppedCard.title}" into group "${targetCard.title}" at index ${nextIndex}`
      )
    } catch (error) {
  ...
  const handleMultiCardDrop = async (droppedSequence: SequenceTile[], targetCard: SequenceTile) => {
    ...
    try {
      ...
      console.log(
        `Moved ${droppedSequence.length} sequence into "${targetCard.title}" starting at index ${nextIndex}`
      )
    } catch (error) {
  ...
  const handleMultiCardReorder = async (reorderedSequence: SequenceTile[], targetIndex: number) => {
    ...
    const remainingSequence = sequence.value.filter(
      c => !reorderedSequence.some(rc => rc.id === c.id)
    )
    ...
    try {
      ...
      const cardIds = newSequence.filter(c => !c.id.startsWith('empty-')).map(c => c.id)
  ...
  const moveSelectedToGroup = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          '[SequenceView] Hierarchical loading failed, using fallback:',
          hierarchicalError
        )
        ...
        const currentLevelSequence = sequence.value.filter(
          c => !c.id.startsWith('empty-') && !selectedTileIds.value.has(c.id)
        )
        ...
        if (currentGroupId.value) {
          try {
            ...
            parentLevelSequence = rootSequence.filter(
              c =>
            ...
            try {
              ...
              for (const card of selectedSequence) {
                await sequenceService.saveCard(
                  ...
                  group.id,
                  nextIndex
                ) // Pass both parentId and index parameters
  ...
  const findFirstEmptyPosition = (): number => {
    ...
    const occupiedPositions = new Set(
      sequence.value.filter(c => !c.id.startsWith('empty-')).map(c => c.index)
    )
        ...
        onSave: async (cardData: Partial<SequenceTile>) => {
          try {
            ...
            if (savedId) {
              ...
              const emptyIndex = sequence.value.findIndex(
                c => c.index === targetIndex && c.id.startsWith('empty-')
              )
              if (emptyIndex !== -1) {
          ...
          try {
            ...
            for (const cardData of newSequence) {
              ...
              if (savedId) {
                ...
                // Update store cache
                await sequenceStore.addCardToCache(
                  newCard,
                  currentGroupId.value,
                  currentLocale.value
                )
  ...
  watch(
    ...
    async (newLocale, oldLocale) => {
      if (newLocale !== oldLocale && newLocale) {
        console.log(
          '[SequenceView] Locale changed from',
          oldLocale,
          'to',
          newLocale,
          '- reloading sequence...'
