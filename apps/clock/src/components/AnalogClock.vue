<template>
	<div :class="bemm()" role="group" :aria-label="ariaLabel">
		...
		>
			...
			<!-- Minute labels (shown when stage requires) -->
			<g v-if="showMinuteLabels">
				<text
			...
			<line
				...
				:class="{ 'analog-clock__hand--highlight': highlight === 'hour' || highlight === 'both' }"
				x1="160"
				y1="160"
				x2="160"
				y2="82"
			...
			<line
				...
				:class="{ 'analog-clock__hand--highlight': highlight === 'minute' || highlight === 'both' }"
				x1="160"
				y1="160"
				x2="160"
				y2="44"
		...
		<div v-if="interactive" :class="bemm('controls')">
...
const props = withDefaults(
	...
	{ ariaLabel: 'Interactive analog clock', interactive: true, highlight: null, showMinuteLabels: false }
)
...
const minuteStep = computed(() =>
	props.stage === 'full-hours' ? 5 : props.stage === 'half-past' ? 5 : 1
)
...
const minuteLabels = Array.from({ length: 12 }, (_, index) => {
	...
	return {
		...
		x: 160 + Math.cos(radians) * 130,
		y: 166 + Math.sin(radians) * 130,
	}
})
...
<style lang="scss">
	.analog-clock {
		...
		&__hand {
			...
			&--highlight.analog-clock__hand--hour {
				stroke-width: 16;
				filter: drop-shadow(0 0 8px rgb(29 78 216 / 0.6));
			}

			&--highlight.analog-clock__hand--minute {
				stroke-width: 11;
				filter: drop-shadow(0 0 8px rgb(234 88 12 / 0.6));
			}
		}
