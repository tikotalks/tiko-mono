  ...
  <div :class="bemm()">
    <div :class="bemm('header')">
      <div :class="bemm('header-actions')">
        <TButton
          v-if="isAdmin && hasBaseContent"
          size="small"
          :class="bemm('generate-all')"
          :icon="Icons.REFRESH"
          :status="isGeneratingAll ? Status.LOADING : undefined"
    ...
    <div :class="bemm('list')">
      ...
      <TCard
        ...
        :key="translation.id || translation.locale"
        :class="
          bemm('item', [
            translation.is_base ? 'base' : '',
            isEditMode(translation) ? 'editing' : 'viewing',
          ])
        "
        :removable="!translation.is_base && isEditMode(translation)"
        @remove="handleDelete(translation)"
      ...
      >
        <div :class="bemm('header')">
          <div :class="bemm('locale')">
            ...
            <TChip v-if="translation.is_base" color="primary" size="small">{{
              t('cards.baseLanguage')
            }}</TChip>
          </div>
          <div v-if="!translation.is_base" :class="bemm('actions')">
            <TButton
              type="ghost"
              size="small"
              :icon="isEditMode(translation) ? Icons.CHECK_M : Icons.EDIT"
        ...
        <div v-if="!isEditMode(translation) || translation.is_base" :class="bemm('view')">
          <span> <TIcon :name="Icons.CLIPBOARD" />{{ translation.name || '-' }} </span>
          <span> <TIcon :name="Icons.SPEECH_BALLOON" />{{ translation.content || '-' }} </span>
        </div>
        ...
        <TFormGroup v-else>
          <TInputText
            ...
            :label="t('cards.titleInLanguage', { language: getLocaleName(translation.locale) })"
            :inline="true"
            :placeholder="
              t('cards.titleInLanguage', { language: getLocaleName(translation.locale) })
            "
            :max-length="50"
          ...
          <TTextarea
            ...
            :label="t('cards.speechInLanguage', { language: getLocaleName(translation.locale) })"
            :inline="true"
            :placeholder="
              t('cards.speechInLanguage', { language: getLocaleName(translation.locale) })
            "
            :rows="2"
            @blur="handleSave(translation)"
          />
    ...
    <TCard v-else :class="bemm('item', ['', 'new', 'editing'])">
      <div :class="bemm('header')">
        <div :class="bemm('locale')">
          <TInputSelect
            ...
            :placeholder="t('cards.selectLanguage')"
      ...
      <TFormGroup>
        <TInputText
          ...
          :label="t('cards.titleInLanguage', { language: getLocaleName(newTranslation.locale) })"
          :inline="true"
          :placeholder="
            t('cards.titleInLanguage', { language: getLocaleName(newTranslation.locale) })
          "
          :max-length="50"
        ...
        <TTextarea
          ...
          :label="t('cards.speechInLanguage', { language: getLocaleName(newTranslation.locale) })"
          :inline="true"
          :placeholder="
            t('cards.speechInLanguage', { language: getLocaleName(newTranslation.locale) })
          "
          :rows="2"
          :disabled="!newTranslation.locale"
        />
        <TButton
          v-if="newTranslation.locale"
          type="primary"
          size="small"
          :icon="Icons.CHECK_M"
          :disabled="!newTranslation.name"
          @click="handleAddNew"
        >
          {{ t('common.save') }}
        </TButton>
...
<script setup lang="ts">
  ...
  const availableLocaleOptions = computed(() => {
    ...
    const baseLanguages = [...new Set(locales.map(locale => locale.split('-')[0]))]
  ...
  const handleDelete = async (translation: ItemTranslationWithBase) => {
    popupService.open({
      ...
      props: {
        ...
        message: t('cards.confirmDeleteTranslation', {
          language: getLocaleName(translation.locale),
        }),
        ...
        onConfirm: async () => {
          try {
            ...
            emit(
              'update:modelValue',
              translations.value.filter(t => !t.is_base)
            )
  ...
  watch(
    () => [props.baseTitle, props.baseSpeech],
    () => {
