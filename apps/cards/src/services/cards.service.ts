...
export const cardsService = {
  ...
  async loadAllCards(includeCurated = false): Promise<CardTile[]> {
    try {
      ...
      console.log(
        '[CardsService] loadAllCards - userId:',
        userId,
        'isSkipAuth:',
        isSkipAuth,
        'includeCurated:',
      ...
      console.log(
        '[CardsService] Loaded',
        items.length,
        'total items from ItemService (including children)'
      )
      ...
      console.log(
        '[CardsService] Flattened to',
        allItems.length,
        'total unique items (deduplicated)'
      )
  ...
  async updateGroupChildrenVisibility(
    groupId: string,
    userId: string,
    isPublic: boolean
  ): Promise<void> {
