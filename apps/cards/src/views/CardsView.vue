...
<script setup lang="ts">
  ...
  const loadCards = async () => {
    try {
      ...
      if (savedCards.length > 0) {
        ...
        const cardsWithChildren = savedCards.filter(
          card => !card.id.startsWith('empty-') && card.has_children === true
        )

        console.log(
          `[CardsView] Loading children for ${cardsWithChildren.length} cards marked with has_children`
        )
        console.log(
          '[CardsView] Cards with has_children flag:',
          cardsWithChildren.map(c => ({ id: c.id, title: c.title, has_children: c.has_children }))
        )
        ...
        for (let i = 0; i < cardsWithChildren.length; i += BATCH_SIZE) {
          ...
          console.log(
            `[CardsView] Processing batch ${batchNumber} of ${totalBatches} (${batch.length} cards)`
          )
            ...
            batch.map(async card => {
              try {
                ...
                if (children.length > 0) {
                  ...
                  console.log(
                    `[CardsView] Loaded ${uniqueChildren.length} unique children for card ${card.id} (${card.title})`
                  )
                } else {
                  // If has_children is true but no children found, it might need fixing
                  console.warn(
                    `[CardsView] Card ${card.id} (${card.title}) marked as has_children but no children found`
                  )
                }
              } catch (error) {
        ...
        if (recentlyModified.length > 0) {
          console.log(
            `[CardsView] Checking ${recentlyModified.length} recently modified cards for children`
          )
          await Promise.all(
            recentlyModified.map(async card => {
              try {
                const children = await yesNoStore.loadCards(card.id, currentLocale.value)
                if (children.length > 0) {
                  newTilesWithChildren.add(card.id)
                  ...
                  if (!newTileChildrenMap.has(card.id)) {
                    ...
                    newTileChildrenMap.set(card.id, uniqueChildren)
                    console.log(
                      `Found ${uniqueChildren.length} unique children for card ${card.id} - has_children flag may need updating`
                    )
                  }
                }
              } catch (error) {
        ...
        console.log(
          '[CardsView] tileChildrenMap:',
          Array.from(tileChildrenMap.value.entries()).map(([id, children]) => ({
            id,
    ...
    popupService.open({
      ...
      props: {
        ...
        onSubmit: async (
          ...
          cardIndex: number,
          newTranslations: ItemTranslation[]
        ) => {
          isSaving.value = true
          if (isNewCard) {
            ...
            const originalChildrenMap =
              card.parentId && tileChildrenMap.value.has(card.parentId)
            ...
            try {
              ...
              await yesNoStore.updateCardInCache(
                updatedCard,
                currentGroupId.value,
                currentLocale.value
              )
              ...
              if (updatedCard.parentId && tileChildrenMap.value.has(updatedCard.parentId)) {
                ...
                if (parentChildren) {
                  ...
                  if (childIndex >= 0) {
                    ...
                    console.log(
                      `Updated child card ${updatedCard.id} in parent ${updatedCard.parentId} children map`
                    )
                  }
                }
              }
  ...
  const duplicateItem = async (originalCard: CardTile) => {
    try {
      ...
      const cardId = await cardsService.saveCard(
        duplicateData,
        originalCard.parentId,
        originalCard.index
      )
  ...
  const handleCardDrop = async (droppedCard: CardTile, targetCard: CardTile) => {
    ...
    try {
      ...
      const allChildren = [...existingChildren, droppedCard].filter(
        (child, index, self) => index === self.findIndex(c => c.id === child.id)
      )
  ...
  const handleMultiCardDrop = async (droppedCards: CardTile[], targetCard: CardTile) => {
    ...
    try {
      ...
      // Ensure no duplicates by filtering based on ID
      const allChildren = [...existingChildren, ...sortedDroppedCards].filter(
        (child, index, self) => index === self.findIndex(c => c.id === child.id)
      )
      tileChildrenMap.value.set(targetCard.id, allChildren)
      ...
      console.log(
        `Moved ${droppedCards.length} cards into "${targetCard.title}" starting at index ${nextIndex}`
      )
    } catch (error) {
  ...
  const handleMultiCardReorder = async (reorderedCards: CardTile[], targetIndex: number) => {
    ...
    try {
      ...
      const cardIds = newCards.filter(c => !c.id.startsWith('empty-')).map(c => c.id)
  ...
  const moveSelectedToGroup = async () => {
    ...
    try {
      ...
      try {
        ...
        const currentLevelCards = cards.value.filter(
        ...
        if (currentGroupId.value) {
          try {
            ...
            parentLevelCards = rootCards.filter(
              c =>
      ...
      popupService.open({
        ...
        props: {
          ...
          onSelect: async (group: CardTile) => {
            try {
              console.log(
                `[CardsView] Moving ${selectedCards.length} cards to group ${group.id} (${group.title})`
              )
              ...
              console.log(
                `[CardsView] Target group has ${existingChildren.length} existing children`
              )

              // Move all selected cards to the target group with sequential indices
              for (const card of selectedCards) {
                console.log(
                  `[CardsView] Moving card ${card.id} (${card.title}) to parent ${group.id} at index ${nextIndex}`
                )
  ...
  const findFirstEmptyPosition = (): number => {
    ...
    const occupiedPositions = new Set(
      cards.value.filter(c => !c.id.startsWith('empty-')).map(c => c.index)
    )
        ...
        onSave: async (cardData: Partial<CardTile>, _index: number) => {
          try {
            ...
            if (savedId) {
              ...
              const emptyIndex = cards.value.findIndex(
        ...
        onCreate: async (newCards: Partial<CardTile>[]) => {
          try {
            ...
            for (const cardData of newCards) {
              ...
              console.log(
                `[CardsView] Creating card "${cardData.title}" with parent:`,
                parentIdToUse
              )
  ...
  watch(
    () => route.params.cardId as string | undefined,
    async cardId => {
      try {
