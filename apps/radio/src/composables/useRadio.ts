...
export function useRadio(): UseRadioReturn {
  ...
  const radioItems = computed<RadioItem[]>(() => 
    items.items.value as RadioItem[]
  )
  ...
  const urlItems = computed<RadioItem[]>(() => 
    radioItems.value.filter(item => item.type === 'url')
  )
  ...
  const getItemByUrl = (url: string): RadioItem | undefined => {
    return radioItems.value.find(item => 
      item.content === url || item.metadata?.url === url
    )
  }
