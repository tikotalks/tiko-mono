  ...
  <div :class="bemm()">
    ...
    <div :class="bemm('filters')">
      ...
      <div :class="bemm('filter-group')">
        ...
        <TInputSelect v-model="filters.app" :options="appOptions" @change="loadItems" />
      </div>
    ...
    <div v-if="selectedIds.size > 0" :class="bemm('selection-actions')">
      ...
      <TButtonGroup>
        <TButton size="small" type="outline" @click="clearSelection">
          {{ t('common.clearSelection') }}
        </TButton>
        <TButton size="small" color="success" @click="bulkSetCurated(true)">
          {{ t('admin.markAsCurated') }}
        </TButton>
        <TButton size="small" color="danger" type="outline" @click="bulkSetCurated(false)">
      ...
      <div v-else :class="bemm('list')">
        ...
        >
          ...
          <div :class="bemm('item-preview')">
            <TCardTile :card="item" :show-image="true" :show-title="true" :edit-mode="false" />
          </div>
...
<script setup lang="ts">
  ...
  const toggleCurated = async (itemId: string, isCurated: boolean) => {
    try {
      ...
      toast?.show({
        message: isCurated ? t('admin.itemMarkedAsCurated') : t('admin.itemRemovedFromCurated'),
        type: 'success',
      })
  ...
  const bulkSetCurated = async (isCurated: boolean) => {
    ...
    try {
      await adminItemsService.bulkToggleCurated(Array.from(selectedIds.value), isCurated)
