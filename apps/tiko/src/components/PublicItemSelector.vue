...
const filteredItems = computed(() => {
  ...
  switch (activeTab.value) {
    case 'curated':
      ...
      break;
    case 'public':
      filtered = filtered.filter(s => s.isPublic && !s.isCurated && s.ownerId !== currentUserId.value);
      break;
    case 'mine':
...
const searchItems = async (query: string) => {
  ...
  try {
    const includeCurated = sequenceStore.settings.showCuratedItems;
    items.value = await sequenceService.searchPublicItems(query, props.itemType, undefined, includeCurated);
  } catch (error) {
