...
<script setup lang="ts">
  ...
  const visibleItems = computed(() => {
    ...
    const items = playState.value.shuffledItems.map(item => {
      const isItemSelected = sequenceStore.currentPlayState.selectedItems.some(
        s => s.id === item.id
      )
    ...
    console.log(
      `[SequencePlay] All items with state:`,
      items.map((item, idx) => ({
        position: idx,
        id: item.id,
        title: item.title,
  ...
  const showRewardOverlay = computed(() => {
    ...
    console.log(
      '[SequencePlay] showRewardOverlay:',
      shouldShow,
      'isComplete:',
      playState.value.isComplete,
      'editMode:',
      props.editMode
    )
    return shouldShow
  ...
  watch(
    ...
    async newId => {
      ...
      if (newId) {
        ...
        console.log(
          `[SequencePlay] Play started, current play state:`,
          sequenceStore.currentPlayState
        )
      }
    },
    { immediate: true }
