      ...
      <div :class="bemm('keyboard')">
        ...
        <div :class="bemm('row')">
          <button
            ...
            :key="key.key"
            :class="[bemm('key'), key.modifier ? bemm('key', 'modifier') : '']"
        ...
        <div v-if="!isNumbersLayout" :class="bemm('row')">
          ...
          <button
            :class="bemm('key', ['', 'backspace'])"
            @click="handleKeyPress(specialKeys.backspace)"
            :disabled="disabled"
          >
            <span :class="bemm('key-text')">
...
<script setup lang="ts">
  ...
  const preloadLayoutAudio = async () => {
    ...
    try {
      ...
      console.log(
        '[VirtualKeyboard] Audio preload complete. Preloaded',
        keysToPreload.length,
        'items'
      )
    } catch (error) {
...
<style lang="scss">
  .virtual-keyboard {
    ...
    &__key {
      --keyboard-key-color-shadow: color-mix(
        in srgb,
        var(--keyboard-key-color),
        var(--color-dark) 50%
      );
      ...
      background-image: radial-gradient(
        var(--keyboard-key-color),
        color-mix(in srgb, var(--keyboard-key-color), var(--color-background) 25%)
      );
      color: var(--keyboard-key-text-color);
      box-shadow: 0 calc(var(--keyboard-shadow-size) * -1) 0 0 var(--keyboard-key-color-shadow)
        inset;
      border: none;
